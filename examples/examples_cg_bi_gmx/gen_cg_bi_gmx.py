...
def gen(gro1_file, gro2_file, itp1_file, itp2_file, geometry, geom_params, lattice_type, lattice_constant, bigraft_pattern, grafting_density, output_dir):
	
	# Debugging check:
	for ifile in [gro1_file, gro2_file, itp1_file, itp2_file]:
		if not os.path.exists(ifile):
			raise FileNotFoundError(f"Whoops! Expected to find {ifile} but it isn't there.")
		
	# import peo
	peo = Polymer("PEO")

	# read  
	peo.readGRO(gro1_file)
	peo.readITP(itp1_file)

	# second graft
	peo6 = Polymer("PEO")
	peo6.readGRO(gro2_file)
	peo6.readITP(itp2_file)
	...
	peo_g_subs.toITP(final_itp_path, with_mass=False)	
